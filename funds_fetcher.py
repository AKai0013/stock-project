...
def main():
    ...
    if not rows:
        ...
        if os.path.exists("data/foreign.csv") and os.path.exists("data/invest.csv"):
            print("使用舊法人資料（fallback）")

            old_foreign = pd.read_csv("data/foreign.csv").to_dict("records")
            old_invest = pd.read_csv("data/invest.csv").to_dict("records")

            save_outputs(
                old_foreign,
                old_invest,
                f"⚠ 使用舊資料（TWSE 抓取失敗）：{err}",
                latest_date
            )
        else:
            save_outputs([], [], f"❌ 無法人資料：{err}", latest_date)
